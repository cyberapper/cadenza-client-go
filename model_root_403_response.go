/*
Algo724 Cadenza Fermata API

OpenAPI Specification for Algo724 Cadenza Fermata API  ## API Resources - <a href=\"/postman-collection.json\" download=\"cadenza-postman-collection.json\">Download Postman Collection</a>

API version: 3.0.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the Root403Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Root403Response{}

// Root403Response struct for Root403Response
type Root403Response struct {
	// Indicates if the operation was successful
	Success bool `json:"success"`
	// Error code (0 for success, negative for errors)
	Errno int32 `json:"errno"`
	// Error message (null for successful operations)
	Error NullableString `json:"error,omitempty"`
	Details NullableBaseResponseDetails `json:"details,omitempty"`
	Data NullableString `json:"data,omitempty"`
}

type _Root403Response Root403Response

// NewRoot403Response instantiates a new Root403Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRoot403Response(success bool, errno int32) *Root403Response {
	this := Root403Response{}
	this.Success = success
	this.Errno = errno
	return &this
}

// NewRoot403ResponseWithDefaults instantiates a new Root403Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRoot403ResponseWithDefaults() *Root403Response {
	this := Root403Response{}
	return &this
}

// GetSuccess returns the Success field value
func (o *Root403Response) GetSuccess() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Success
}

// GetSuccessOk returns a tuple with the Success field value
// and a boolean to check if the value has been set.
func (o *Root403Response) GetSuccessOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Success, true
}

// SetSuccess sets field value
func (o *Root403Response) SetSuccess(v bool) {
	o.Success = v
}

// GetErrno returns the Errno field value
func (o *Root403Response) GetErrno() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Errno
}

// GetErrnoOk returns a tuple with the Errno field value
// and a boolean to check if the value has been set.
func (o *Root403Response) GetErrnoOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Errno, true
}

// SetErrno sets field value
func (o *Root403Response) SetErrno(v int32) {
	o.Errno = v
}

// GetError returns the Error field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Root403Response) GetError() string {
	if o == nil || IsNil(o.Error.Get()) {
		var ret string
		return ret
	}
	return *o.Error.Get()
}

// GetErrorOk returns a tuple with the Error field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Root403Response) GetErrorOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Error.Get(), o.Error.IsSet()
}

// HasError returns a boolean if a field has been set.
func (o *Root403Response) HasError() bool {
	if o != nil && o.Error.IsSet() {
		return true
	}

	return false
}

// SetError gets a reference to the given NullableString and assigns it to the Error field.
func (o *Root403Response) SetError(v string) {
	o.Error.Set(&v)
}
// SetErrorNil sets the value for Error to be an explicit nil
func (o *Root403Response) SetErrorNil() {
	o.Error.Set(nil)
}

// UnsetError ensures that no value is present for Error, not even an explicit nil
func (o *Root403Response) UnsetError() {
	o.Error.Unset()
}

// GetDetails returns the Details field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Root403Response) GetDetails() BaseResponseDetails {
	if o == nil || IsNil(o.Details.Get()) {
		var ret BaseResponseDetails
		return ret
	}
	return *o.Details.Get()
}

// GetDetailsOk returns a tuple with the Details field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Root403Response) GetDetailsOk() (*BaseResponseDetails, bool) {
	if o == nil {
		return nil, false
	}
	return o.Details.Get(), o.Details.IsSet()
}

// HasDetails returns a boolean if a field has been set.
func (o *Root403Response) HasDetails() bool {
	if o != nil && o.Details.IsSet() {
		return true
	}

	return false
}

// SetDetails gets a reference to the given NullableBaseResponseDetails and assigns it to the Details field.
func (o *Root403Response) SetDetails(v BaseResponseDetails) {
	o.Details.Set(&v)
}
// SetDetailsNil sets the value for Details to be an explicit nil
func (o *Root403Response) SetDetailsNil() {
	o.Details.Set(nil)
}

// UnsetDetails ensures that no value is present for Details, not even an explicit nil
func (o *Root403Response) UnsetDetails() {
	o.Details.Unset()
}

// GetData returns the Data field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Root403Response) GetData() string {
	if o == nil || IsNil(o.Data.Get()) {
		var ret string
		return ret
	}
	return *o.Data.Get()
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Root403Response) GetDataOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data.Get(), o.Data.IsSet()
}

// HasData returns a boolean if a field has been set.
func (o *Root403Response) HasData() bool {
	if o != nil && o.Data.IsSet() {
		return true
	}

	return false
}

// SetData gets a reference to the given NullableString and assigns it to the Data field.
func (o *Root403Response) SetData(v string) {
	o.Data.Set(&v)
}
// SetDataNil sets the value for Data to be an explicit nil
func (o *Root403Response) SetDataNil() {
	o.Data.Set(nil)
}

// UnsetData ensures that no value is present for Data, not even an explicit nil
func (o *Root403Response) UnsetData() {
	o.Data.Unset()
}

func (o Root403Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Root403Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["success"] = o.Success
	toSerialize["errno"] = o.Errno
	if o.Error.IsSet() {
		toSerialize["error"] = o.Error.Get()
	}
	if o.Details.IsSet() {
		toSerialize["details"] = o.Details.Get()
	}
	if o.Data.IsSet() {
		toSerialize["data"] = o.Data.Get()
	}
	return toSerialize, nil
}

func (o *Root403Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"success",
		"errno",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varRoot403Response := _Root403Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varRoot403Response)

	if err != nil {
		return err
	}

	*o = Root403Response(varRoot403Response)

	return err
}

type NullableRoot403Response struct {
	value *Root403Response
	isSet bool
}

func (v NullableRoot403Response) Get() *Root403Response {
	return v.value
}

func (v *NullableRoot403Response) Set(val *Root403Response) {
	v.value = val
	v.isSet = true
}

func (v NullableRoot403Response) IsSet() bool {
	return v.isSet
}

func (v *NullableRoot403Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRoot403Response(val *Root403Response) *NullableRoot403Response {
	return &NullableRoot403Response{value: val, isSet: true}
}

func (v NullableRoot403Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRoot403Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


